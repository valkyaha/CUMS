...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn parse_header(data: &[u8]) -> (u32, u32, u32, u32, u32, u32, u32, u32) {
        ...
        (version, sample_count, sample_headers_size, name_table_size, data_size, codec, zero, flags)
    }
    ...
    println!("Sample headers:     {:10}  {:10}  {:+}", osh, msh, msh as i64 - osh as i64);
    println!("Name table size:    {:10}  {:10}  {:+}", ont, mnt, mnt as i64 - ont as i64);
    println!("Data size:          {:10}  {:10}  {:+}", ods, mds, mds as i64 - ods as i64);
    ...
    println!("\nData offset:        {:10}  {:10}  {:+}", orig_data_offset, mod_data_offset, mod_data_offset as i64 - orig_data_offset as i64);
    ...
    for i in 0..3 {
        ...
        println!("  Orig: {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X}",
            orig[orig_start], orig[orig_start+1], orig[orig_start+2], orig[orig_start+3],
            orig[orig_start+4], orig[orig_start+5], orig[orig_start+6], orig[orig_start+7]);
        println!("  Mod:  {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X}",
            modded[mod_start], modded[mod_start+1], modded[mod_start+2], modded[mod_start+3],
            modded[mod_start+4], modded[mod_start+5], modded[mod_start+6], modded[mod_start+7]);
    }
    ...
    println!("Original name table starts at offset {} (0x{:X})", orig_nt_start, orig_nt_start);
    println!("Modified name table starts at offset {} (0x{:X})", mod_nt_start, mod_nt_start);
    ...
    println!("\n=== Data Section Start ===\n");
    println!("Original data starts at offset {} (0x{:X})", orig_data_offset, orig_data_offset);
    println!("Modified data starts at offset {} (0x{:X})", mod_data_offset, mod_data_offset);
